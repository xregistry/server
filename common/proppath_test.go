...
func TestParseUI(t *testing.T) {
	...
	tests := []Test{
		...
		{"['prop1.prop2'].prop3", `{[{"prop1.prop2",-1},{"prop3",-1}]}`},
		{"['prop1-prop2']", `{[{"prop1-prop2",-1}]}`},
		{"['1']", `{[{"1",-1}]}`},
		{"['*']", `{[{"*",-1}]}`},
		...
		// Wildcards
		{"*", `{[{"*",-1,"IsWild":true}],"HasWild":true}`},
		{"a1.*", `{[{"a1",-1},{"*",-1,"IsWild":true}],"HasWild":true}`},
		{"a1.*.a3", `{[{"a1",-1},{"*",-1,"IsWild":true},{"a3",-1}],"HasWild":true}`},
		{"a1[*]", `{[{"a1",-1},{"*",-2,"IsWild":true}],"HasWild":true}`},
		{"a1[*].a3", `{[{"a1",-1},{"*",-2,"IsWild":true},{"a3",-1}],"HasWild":true}`},
		{"a1[*].*", `{[{"a1",-1},{"*",-2,"IsWild":true},{"*",-1,"IsWild":true}],"HasWild":true}`},
		...
		{"a1[']", `Unexpected "]" in "a1[']" at pos 5`},
		{"a1['a'']", `Unexpected "'" in "a1['a'']" at pos 7`},
		{"a1['a']'", `Unexpected "'" in "a1['a']'" at pos 8`},

		{`a1[""]`, `Unexpected '"' in "a1[\"\"]" at pos 5`},
		{`a1["]`, `Unexpected "]" in "a1[\"]" at pos 5`},
		{`a1["a""]`, `Unexpected '"' in "a1[\"a\"\"]" at pos 7`},
		{`a1["a"]"`, `Unexpected '"' in "a1[\"a\"]\"" at pos 8`},
		{`a1["a"].["b"]`, `Unexpected "[" in "a1[\"a\"].[\"b\"]" at pos 9`},

		{"a1.a*", `Unexpected "*" in "a*"`},
		{"a1.a*.def", `Unexpected "*" in "a*"`},
		{"a1.*a", `Unexpected "*" in "*a"`},
		{"a1.*a.def", `Unexpected "*" in "*a"`},
		{"a1[a*]", `Expecting an integer at pos 4 in "a1[a*]"`},
		{"a1[a*].def", `Expecting an integer at pos 4 in "a1[a*].def"`},
		{"a1[2*]", `Unexpected "*" in "2*"`},
		{"a1[2*].def", `Unexpected "*" in "2*"`},
		{"a1[*2]", `Unexpected "*" in "*2"`},
		{"a1[*2].def", `Unexpected "*" in "*2"`},
		{"a1.*2.def", `Unexpected "*" in "*2"`},
		{"a1.2*.def", `Unexpected "*" in "2*"`},
	...
	for _, test := range tests {
		...
		if pp != nil {
			...
			res = strings.ReplaceAll(res, `,"IsWild":false`, "")
			res = strings.ReplaceAll(res, `,"HasWild":false`, "")
		} else {
			res = err.Error()
		}
		if res != test.Exp {
