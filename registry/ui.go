...
`)
	...
	for _, name := range regs {
		...
		if info.ProxyHost != "" && strings.Contains(info.ProxyHost, name) {
			...
			checked = " selected"
		} else if name == "Default" && !strings.Contains(info.BaseURL, "/"+
			RegCollectionSegment) && info.ProxyHost == "" {
			selectedRegistry = name
			checked = " selected"
		} else if strings.Contains(info.BaseURL,
			"/"+RegCollectionSegment+"/"+name) {
...
function changeRegistry(name) {
  ...
  } else loc = "/` + RegCollectionSegment + `/" + name + "?ui"
