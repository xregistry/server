...
// ---- Canonical-order JSON pretty-printer (TODO.md "json-pretty-printer") --
//
// canonicalPrettyPrintXRegJSON() is the JS mirror of the Go
// CanonicalPrettyPrintJSON() utility in common/shared_entity: given
// arbitrary xRegistry JSON (a single entity, or a full nested Registry
// doc, possibly captured from another server), it reorders every nested
// entity's own attributes into the spec's canonical order (core/spec.md
// "Design: JSON Serialization"), using SPEC_ATTRS_CANONICAL_ORDER (see
// specattrs.js, generated by cmds/genspecattrs from the same
// registry.OrderedSpecProps table the Go printer uses) as the ordering
// source of truth, so both languages can never silently diverge.
//
// This is a standalone utility function only — nothing in the SPA calls
// it yet (see plan.md/TODO.md; wiring it into the JSON-view/download
// button is a deliberately deferred follow-up task).
//
// Entities are found structurally: any JSON object with its own string
// "xid" field is classified (by depth of its xid path — see
// canonicalXidInfo()) and its own keys are reordered; every other
// nested object/array keeps its own original key order but is still
// recursed into (so a collection of entities, e.g. a map of Group
// instances, or a Resource's "versions" map, gets each child reordered
// even though the collection wrapper itself isn't touched). Extension
// (non-spec) attributes are alphabetized at each level's "$extensions"
// slot; blank lines are emitted wherever a "$space" marker survives.
//
// Model-independent heuristics (mirroring the Go implementation, see
// common/pretty.go's writeCanonicalEntity() for the fuller rationale):
//   - Group/Resource/Meta/Version's own "id" attribute's real wire name
//     (e.g. "messageid") is resolved by VALUE — the entity's own xid
//     path segment is matched against each of its unconsumed "...id"
//     keys' values — not by guessing a name. Only the Registry's own
//     "id" has a fixed, model-independent wire name ("registryid"),
//     already baked into SPEC_ATTRS_CANONICAL_ORDER.
//   - A Version's "$RESOURCE"-family attrs (<singular>url/proxyurl/
...
//     sibling is treated as one collection.
//   - If none of the above heuristics find a match, the token is simply
//     skipped and the real data (if present under some other name)
//     falls through to being treated as an ordinary (alphabetized)
//     extension instead.
//
// Accepts either a JSON string or an already-parsed JS value; always
// returns a formatted string (or throws, if input is a string that isn't
// valid JSON).
...
// canonicalXidInfo parses a xid string into { level, groupID, resourceID }
// (level: 'registry'/'group'/'resource'/'meta'/'version'), or null if xid
// isn't a usable entity xid. Purely structural (path-depth based), same
// as common.ParseXid() on the Go side — doesn't require or consult any
// model. groupID/resourceID are the entity's own path-segment IDs (e.g.
// for "/dirs/d1", groupID="d1"), used to resolve the entity's own generic
// "id" attribute by VALUE (see resolveIDKeyByValue()).
function canonicalXidInfo(xid) {
  if (typeof xid !== 'string' || xid === '' || xid.charAt(0) !== '/') return null;
  var s = xid;
  if (s.length > 1 && s.charAt(s.length - 1) === '/') s = s.slice(0, -1);
  var parts = s === '/' ? [] : s.slice(1).split('/');
  for (var i = 0; i < parts.length; i++) {
    if (parts[i] === '') return null;
  }
  if (parts.length === 0) return { level: 'registry' };
  if (parts.length === 2) return { level: 'group', groupID: parts[1] };
  if (parts.length === 4) return { level: 'resource', groupID: parts[1], resourceID: parts[3] };
  if (parts.length === 5) {
    if (parts[4] === 'meta') return { level: 'meta', groupID: parts[1], resourceID: parts[3] };
    return null; // ".../versions" (collection, no ID) isn't an entity
  }
  if (parts.length === 6 && parts[4] === 'versions') {
    return { level: 'version', groupID: parts[1], resourceID: parts[3], versionID: parts[5] };
  }
  return null;
}

// writeCanonicalValue recursively appends val's canonical-order lines
// (indented by indent) to lines. Mirrors writeCanonicalValue/
// writeCanonicalEntity/writePlainMap/writeCanonicalArray in
// common/pretty.go.
function writeCanonicalValue(val, indent, lines) {
  if (Array.isArray(val)) {
    writeCanonicalArray(val, indent, lines);
    return;
  }
  if (val !== null && typeof val === 'object') {
    var info = canonicalXidInfo(val.xid);
    if (info && typeof SPEC_ATTRS_CANONICAL_ORDER !== 'undefined' && SPEC_ATTRS_CANONICAL_ORDER[info.level]) {
      writeCanonicalEntity(val, indent, SPEC_ATTRS_CANONICAL_ORDER[info.level], info, lines);
    } else {
      writePlainMap(val, indent, lines);
    }
    return;
  }
...
function writeCanonicalArray(arr, indent, lines) {
  if (arr.length === 0) {
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    writeCanonicalValue(arr[i], childIndent, lines);
    if (i < arr.length - 1) lines[lines.length - 1] += ',';
  }
  lines.push(indent + ']');
}

// writePlainMap renders obj with its OWN original key order preserved
// (no canonical ordering rule applies — e.g. "labels", "capabilities",
// "model", or a collection map keyed by entity ID) — but each value is
// still recursed into, so nested entities (e.g. each Group instance
// inside a Groups collection map) still get their own attributes
// reordered.
function writePlainMap(obj, indent, lines) {
  ...
  for (var i = 0; i < keys.length; i++) {
    lines.push(childIndent + JSON.stringify(keys[i]) + ': ');
    writeCanonicalValue(obj[keys[i]], childIndent, lines);
    if (i < keys.length - 1) lines[lines.length - 1] += ',';
  }
  lines.push(indent + '}');
}

// writeCanonicalEntity renders obj (an object with its own valid "xid")
// in canonical order: spec attrs from tokens (in order, skipped if
// absent from obj), a "$space" token emits a blank output line.
//
// Three token kinds need special handling because their real wire-format
// key name depends on the actual (usually unavailable) model — see
// resolveIDKeyByValue()/detectHeuristicCollections() below and the fuller
// explanation in common/pretty.go's writeCanonicalEntity():
...
//     resolved by VALUE via info.groupID/info.resourceID, not by
//     guessing a name.
...
//
// The first "$extensions" token expands to every one of obj's OWN keys
// not otherwise consumed by a spec-attr token or by one of the above,
// alphabetized (any further "$extensions" token, which shouldn't
// normally occur, is a no-op). If none of the heuristics above find a
// match, the corresponding token is simply skipped and the real data —
// if present under some other name — falls through to being treated as
// an ordinary (alphabetized) extension instead.
function writeCanonicalEntity(obj, indent, tokens, info, lines) {
  var consumed = {};
  for (var i = 0; i < tokens.length; i++) {
    var t = tokens[i];
    if (t === '$space' || t === '$extensions' || t.charAt(0) === '$') continue;
    consumed[t] = true;
  }

  var idInfo = resolveIDKeyByValue(obj, info, consumed);
  var resolvedIDKey = idInfo.key, singular = idInfo.singular;
  if (resolvedIDKey) consumed[resolvedIDKey] = true;

  var hasCollectionsToken = tokens.indexOf('$COLLECTIONS') >= 0;
  var hasResourceToken = tokens.indexOf('$RESOURCEurl') >= 0;
  ...
  if (hasCollectionsToken) {
    if (info.level === 'resource') {
      ...
      collectionsKeys = detectHeuristicCollections(obj, consumed);
      collectionsKeys.forEach(function (k) { consumed[k] = true; });
    }
  }

  var resourceFamilyKeys = [];
  if (hasResourceToken && singular) {
    ['url', 'proxyurl', '', 'base64'].forEach(function (suffix) {
      var k = singular + suffix;
      if (Object.prototype.hasOwnProperty.call(obj, k)) {
        resourceFamilyKeys.push(k);
        consumed[k] = true;
      }
    });
  }
  ...
  for (var i = 0; i < objKeys.length; i++) {
    if (!consumed[objKeys[i]]) extNames.push(objKeys[i]);
  }
  extNames.sort();

  // Build the emission plan: real keys to emit, or blank-line markers.
  var plan = []; // { blank: bool, key: string }
  var extensionsEmitted = false;
  for (var i = 0; i < tokens.length; i++) {
    var t = tokens[i];
    if (t === '$space') {
    ...
    if (t === '$extensions') {
      if (!extensionsEmitted) {
        ...
        for (var j = 0; j < extNames.length; j++) plan.push({ key: extNames[j] });
      }
      continue;
    }
    if (t === '$COLLECTIONS') {
      for (var j = 0; j < collectionsKeys.length; j++) plan.push({ key: collectionsKeys[j] });
      continue;
    }
    if (t === '$RESOURCEurl') {
      for (var j = 0; j < resourceFamilyKeys.length; j++) plan.push({ key: resourceFamilyKeys[j] });
      continue;
    }
    if (t === '$RESOURCEproxyurl' || t === '$RESOURCE' || t === '$RESOURCEbase64') {
      continue; // already emitted via the "$RESOURCEurl" slot above
    }
    if (t === 'id') {
      if (resolvedIDKey) plan.push({ key: resolvedIDKey });
      continue;
    }
    if (t.charAt(0) === '$') continue; // unresolvable placeholder — no-op
    if (Object.prototype.hasOwnProperty.call(obj, t)) plan.push({ key: t });
  }
  ...
  for (var i = 0; i < plan.length; i++) {
    var p = plan[i];
    if (p.blank) {
      if (collapsed.length === 0 || collapsed[collapsed.length - 1].blank) continue;
    }
    collapsed.push(p);
  }
  while (collapsed.length > 0 && collapsed[collapsed.length - 1].blank) {
    collapsed.pop();
  }
  ...
  for (var i = collapsed.length - 1; i >= 0; i--) {
    if (!collapsed[i].blank) { lastRealIdx = i; break; }
  }
  for (var i = 0; i < collapsed.length; i++) {
    ...
    lines.push(childIndent + JSON.stringify(p.key) + ': ');
    writeCanonicalValue(obj[p.key], childIndent, lines);
    if (i !== lastRealIdx) lines[lines.length - 1] += ',';
  }
  lines.push(indent + '}');
}

// resolveIDKeyByValue finds obj's own generic "id" attribute by VALUE
// (mirrors common/pretty.go's resolveIDKeyByValue()): looks for an
// unconsumed key ending in "id" whose value equals info.groupID (Group
// level) or info.resourceID (Resource/Meta/Version levels). Registry
// level's "id" is already hardcoded to "registryid" in the tokens table,
// so this always returns no match for Registry. Returns
// { key, singular } — key is '' if no match found; singular is the
// key with its trailing "id" stripped off (e.g. "messageid" -> "message"),
// used to resolve the "$RESOURCE*" family at the Version level.
function resolveIDKeyByValue(obj, info, consumed) {
  if (!info) return { key: '', singular: '' };
  var want;
  if (info.level === 'group') want = info.groupID;
  else if (info.level === 'resource' || info.level === 'meta' || info.level === 'version') want = info.resourceID;
  else return { key: '', singular: '' };
  if (want === undefined) return { key: '', singular: '' };

  var keys = Object.keys(obj);
  for (var i = 0; i < keys.length; i++) {
    var k = keys[i];
    if (consumed[k]) continue;
    if (k.length <= 2 || k.slice(-2) !== 'id') continue;
    if (obj[k] === want) {
      return { key: k, singular: k.slice(0, -2) };
    }
  }
  return { key: '', singular: '' };
}

// detectHeuristicCollections scans obj's own unconsumed keys for
// "<name>count" keys that have a matching unconsumed "<name>url"
// sibling, i.e. a group-type/resource-type collection pair (mirrors
// common/pretty.go's detectHeuristicCollections()). Registry/Group
...
function detectHeuristicCollections(obj, consumed) {
  ...
  for (var i = 0; i < keys.length; i++) {
    var k = keys[i];
    if (consumed[k]) continue;
    if (k.length <= 5 || k.slice(-5) !== 'count') continue;
    var base = k.slice(0, -5);
    if (base === '') continue;
    var urlKey = base + 'url';
    if (consumed[urlKey] || !Object.prototype.hasOwnProperty.call(obj, urlKey)) continue;
    bases.push(base);
  }
  bases.sort();
  var out = [];
